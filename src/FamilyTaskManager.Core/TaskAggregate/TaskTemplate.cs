...
public class TaskTemplate : EntityBase<TaskTemplate, Guid>, IAggregateRoot
{
  ...
  public TaskTemplate(Guid familyId, Guid petId, string title, int points, Schedule schedule, TimeSpan dueDuration,
    Guid createdBy)
  {
  ...
  public void Update(string title, int points, Schedule schedule, TimeSpan dueDuration)
  {
    Guard.Against.NullOrWhiteSpace(title);
    Guard.Against.OutOfRange(points, nameof(points), 1, 100);
    Guard.Against.Null(schedule);
