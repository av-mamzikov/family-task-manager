# 🤖 Настройка Telegram бота

Пошаговая инструкция по созданию и настройке Telegram бота для Family Task Manager.

## Создание бота через @BotFather

1. **Откройте Telegram** и найдите [@BotFather](https://t.me/BotFather)

2. **Отправьте команду** `/newbot`

3. **Выберите имя бота** (отображается в списке чатов)
   ```
   Family Task Manager
   ```

4. **Выберите username** (должен заканчиваться на `bot`)
   ```
   family_task_manager_bot
   ```

5. **Скопируйте токен** - он понадобится для настройки
   ```
   1234567890:ABCdefGHIjklMNOpqrsTUVwxyz
   ```

## Настройка команд бота

Отправьте `/setcommands` в @BotFather, выберите вашего бота и вставьте:

```
start - Начать работу с ботом
family - Управление семьей
tasks - Мои задачи
pet - Мой питомец
stats - Статистика
help - Помощь
```

## Настройка описания

### Короткое описание

Отправьте `/setdescription` в @BotFather:

```
🏠 Семейный менеджер задач
Создавайте задачи, назначайте их членам семьи и следите за выполнением. Заводите питомца и получайте очки за выполнение задач!
```

### О боте (About)

Отправьте `/setabouttext` в @BotFather:

```
Family Task Manager - семейный менеджер задач с геймификацией.

Возможности:
• Управление задачами для всей семьи
• Виртуальные питомцы
• Система очков и лидерборд
• Периодические задачи по расписанию

Разработано для мотивации детей через игровые механики.
```

## Настройка изображения (опционально)

1. Создайте или найдите изображение 512x512 px
2. Отправьте `/setuserpic` в @BotFather
3. Загрузите изображение

## Использование токена

### Для локальной разработки

```bash
cd src/FamilyTaskManager.Host
dotnet user-secrets set "Bot:BotToken" "YOUR_BOT_TOKEN"
dotnet user-secrets set "Bot:BotUsername" "your_bot_username"
```

### Для production (GitHub Secrets)

1. Перейдите в репозиторий: `Settings` → `Secrets and variables` → `Actions`
2. Создайте секрет `TELEGRAM_BOT_TOKEN` со значением токена
3. Создайте секрет `TELEGRAM_BOT_USERNAME` с username бота (без @)

## Создание тестового бота для PR Preview

Для тестирования изменений в изолированном окружении создайте отдельного бота:

1. Повторите шаги выше с другим именем:
   ```
   Family Task Manager Test
   family_task_manager_test_bot
   ```

2. Добавьте в GitHub Secrets:
    - `PR_BOT_TOKEN` - токен тестового бота
    - `PR_BOT_USERNAME` - username тестового бота

## Проверка работы бота

### Локально

```bash
cd src/FamilyTaskManager.Host
dotnet run
```

Откройте бота в Telegram и отправьте `/start`

### На production

После деплоя откройте production бота и отправьте `/start`

## Команды бота

### Основные команды

| Команда   | Описание                                            |
|-----------|-----------------------------------------------------|
| `/start`  | Начать работу с ботом, регистрация                  |
| `/family` | Управление семьей (создать, присоединиться, список) |
| `/tasks`  | Мои активные задачи                                 |
| `/pet`    | Мой питомец (создать, посмотреть, покормить)        |
| `/stats`  | Статистика и лидерборд                              |
| `/help`   | Справка по командам                                 |

### Для администраторов семьи

- Создание периодических задач
- Управление участниками
- Настройка параметров семьи

## Настройки конфиденциальности

### Рекомендуемые настройки

Отправьте следующие команды в @BotFather:

1. **Разрешить добавление в группы** (опционально)
   ```
   /setjoingroups
   ```
   Выберите: `Enable` или `Disable`

2. **Режим группы** (если бот в группах)
   ```
   /setprivacy
   ```
   Выберите: `Enable` (бот видит только команды) или `Disable` (бот видит все сообщения)

## Webhook vs Long Polling

### Long Polling (текущий режим)

**Преимущества:**

- ✅ Проще настройка
- ✅ Не требует HTTPS
- ✅ Работает за NAT

**Недостатки:**

- ❌ Больше нагрузка на сервер
- ❌ Задержка в получении сообщений

### Webhook (для production)

**Преимущества:**

- ✅ Меньше нагрузка
- ✅ Мгновенная доставка
- ✅ Лучше для масштабирования

**Недостатки:**

- ❌ Требует HTTPS
- ❌ Требует публичный IP/домен

**Настройка webhook (будущее):**

```bash
curl -F "url=https://yourdomain.com/bot/webhook" \
     https://api.telegram.org/bot<TOKEN>/setWebhook
```

## Мониторинг

### Проверка статуса бота

```bash
curl https://api.telegram.org/bot<TOKEN>/getMe
```

### Получение обновлений

```bash
curl https://api.telegram.org/bot<TOKEN>/getUpdates
```

### Логи бота

```bash
# Локально
dotnet run

# На VPS
docker compose -f docker-compose.prod.yml logs family-task-manager -f
```

## Troubleshooting

### Бот не отвечает

1. **Проверьте токен:**
   ```bash
   curl https://api.telegram.org/bot<TOKEN>/getMe
   ```

2. **Проверьте логи:**
   ```bash
   docker compose logs family-task-manager -f
   ```

3. **Проверьте, что бот запущен:**
   ```bash
   docker compose ps
   ```

### Бот отвечает с задержкой

- Проверьте нагрузку на сервер
- Проверьте логи на ошибки
- Рассмотрите переход на webhook

### Команды не работают

1. Проверьте, что команды настроены в @BotFather
2. Проверьте логи на ошибки обработки
3. Убедитесь, что пользователь зарегистрирован (`/start`)

## Безопасность

### Защита токена

- ❌ **Никогда** не коммитьте токен в git
- ✅ Используйте user-secrets для локальной разработки
- ✅ Используйте GitHub Secrets для CI/CD
- ✅ Используйте environment variables на production

### Ротация токена

Если токен скомпрометирован:

1. Отправьте `/revoke` в @BotFather
2. Получите новый токен
3. Обновите секреты в GitHub
4. Передеплойте приложение

## Лимиты Telegram Bot API

- **Сообщения:** 30 сообщений/секунду
- **Одному пользователю:** 1 сообщение/секунду
- **Размер сообщения:** 4096 символов
- **Размер файла:** 50 MB (для ботов)

## Дополнительные ресурсы

- [Telegram Bot API Documentation](https://core.telegram.org/bots/api)
- [BotFather Commands](https://core.telegram.org/bots#botfather)
- [Telegram.Bot Library](https://github.com/TelegramBots/Telegram.Bot)

---

**Следующие шаги:**

- 🖥️ [Локальная разработка](setup/LOCAL_DEVELOPMENT.md) — запуск проекта через AspireHost
- 🖥️ [VPS Setup](setup/VPS_SETUP.md) — настройка production сервера
- 🔐 [Secrets Setup](setup/SECRETS_SETUP.md) — установка секретов в GitHub
