...
2. **Проверьте pgAdmin:** откройте URL ресурса pgAdmin из Aspire Dashboard (порт на хосте выбирается случайный
   свободный, если вы не фиксировали его явно)
...
- ✅ **PostgreSQL** контейнер (порт 5432 внутри контейнера, случайный свободный порт на хосте по умолчанию)
- ✅ **pgAdmin** веб-интерфейс (порт 80 внутри контейнера, случайный свободный порт на хосте по умолчанию)
...
По умолчанию .NET Aspire для ресурса PostgreSQL открывает **случайный свободный порт на хосте**, а внутри контейнера
используется стандартный порт `5432`. Поэтому конфликтов порта `5432` на хосте обычно не возникает.

Если вам нужно **зафиксировать конкретный порт на хосте** (например, `5433`), это можно сделать в `Program.cs` проекта
`FamilyTaskManager.AspireHost` в месте, где настраивается PostgreSQL:
...
Чтобы использовать другой порт PostgreSQL (например, `5433`), добавьте конфигурацию порта к ресурсу PostgreSQL в этом же
месте, следуя документации .NET Aspire (метод `WithEndpoint` для Postgres‑ресурса), чтобы явно задать проброс порта на
хост.

### Изменение порта pgAdmin

pgAdmin подключается через расширение `.WithPgAdmin()` для ресурса PostgreSQL в `Program.cs` AspireHost. Если порт
`5050` занят, измените порт pgAdmin в конфигурации Postgres/pgAdmin в `Program.cs`, руководствуясь документацией .NET
Aspire по настройке портов дополнительных ресурсов для PostgreSQL.

### Отключение pgAdmin

Если pgAdmin не нужен при локальной разработке, удалите вызов `.WithPgAdmin()` у ресурса PostgreSQL в `Program.cs`
AspireHost или временно закомментируйте эту часть строки.

---

## 🗄️ Копия продакшен-базы для локальной разработки

Иногда полезно запустить приложение на **реалистичных данных**, близких к production. Рекомендуемый подход — *
*скопировать продакшен-базу** (или её реплику) в локальное окружение.

> ⚠️ **Важно:** перед копированием убедитесь, что это не нарушает политику работы с персональными данными. При
> необходимости анонимизируйте данные после импорта.

### Вариант 1: через DBeaver (дамп → импорт)

1. **Сделайте дамп продакшен-БД**
    - Подключитесь к продакшен-БД (или её реплике) в DBeaver
    - Правый клик по базе → `Tools` → `Backup` / `Dump database`
    - Выберите формат `custom`/`tar` и сохраните файл, например `prod_backup.dump`

2. **Найдите параметры локального PostgreSQL Aspire**
    - Запустите `FamilyTaskManager.AspireHost`
    - В Aspire Dashboard откройте ресурс PostgreSQL
    - Посмотрите:
        - Host: `localhost`
        - Port: внешний порт контейнера (случайный, назначен Aspire)
        - Database: `FamilyTaskManager`
        - User/Password: `postgres` / `postgres`

3. **Подготовьте локальную БД к восстановлению**
    - В Aspire Dashboard остановите ресурс **Host**, чтобы снять активные подключения к базе `FamilyTaskManager`
    - В DBeaver откройте **Session Manager** и закройте/завершите все активные сессии к базе `FamilyTaskManager`

4. **Импортируйте дамп в локальную БД**
    - Подключитесь в DBeaver к локальному Postgres (host/port из Aspire Dashboard)
    - Правый клик по базе `FamilyTaskManager` → `Tools` → `Restore`
    - Укажите файл `prod_backup.dump`, при необходимости включите опцию очистки схемы перед восстановлением

После этого локальная БД Aspire будет содержать структуру и данные из продакшена (или его копии), и Host, запущенный
через AspireHost, автоматически начнёт работать с этой БД.

### Вариант 2: отдельная локальная БД «prod copy»

Если не хотите затирать обычную dev-БД:

1. Создайте в локальном Postgres новую БД, например `FamilyTaskManager_ProdCopy` (через DBeaver или pgAdmin)
2. Восстановите в неё дамп так же, как в варианте 1
3. Настройте для Host-а отдельную строку подключения к этой базе (через `appsettings.*.json` или переменные окружения),
   если нужно запускать Host именно против копии продакшен-БД
...
**Примечание:** При стандартной конфигурации через .NET Aspire PostgreSQL публикуется на случайный свободный порт хоста,
поэтому такая ошибка маловероятна. Она может появиться, если вы **жёстко зафиксировали порт** при настройке эндпоинта.

**Причина:** В явно заданной конфигурации эндпоинта порт PostgreSQL на хосте совпал с уже занятым портом.
...
**Примечание:** При стандартной конфигурации через `.WithPgAdmin()` pgAdmin публикуется на случайный свободный порт
хоста. Порт `5050` используется как пример/целевой порт, если вы явно зафиксировали его в конфигурации.

**Причина:** В явно заданной конфигурации порт pgAdmin на хосте совпал с уже занятым портом.
...
**URL:** URL ресурса pgAdmin, указанный в Aspire Dashboard (порт на хосте выбирается случайный свободный, если вы не
фиксировали его явно)
