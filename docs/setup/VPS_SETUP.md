...
После завершения скрипта вы получите все необходимые данные (хост, логины, пароли и ключи).
Их нужно сохранить и добавить в GitHub Secrets, чтобы пайплайны могли деплоить приложение.
...
Перейдите в репозиторий: `Settings` → `Secrets and variables` → `Actions` → `New repository secret` и добавьте секреты, которые вы получили после работы `init-vps.sh`.

- Полный список секретов и примеры значений: [Secrets Setup Guide](SECRETS_SETUP.md)
- Как именно эти секреты используются в GitHub Actions (production / PR Preview): [GitHub Actions Setup](GITHUB_ACTIONS_SETUP.md)
...
Для pull request можно поднимать отдельное PR Preview окружение (бот + БД, изолированные от production).

Подробная документация по CI/CD, GitHub Actions, меткам `deploy-preview` и `clean-db`, а также комментариям в PR — в разделе
[CI/CD и PR Preview](GITHUB_ACTIONS_SETUP.md).
