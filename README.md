...
- [Локальное тестирование](LOCAL_TESTING.md) — тестирование Docker образа и production конфигурации
...
- 📊 **[Сводка по развертыванию](DEPLOYMENT_SUMMARY.md)** — обзор всех вариантов и текущего состояния
- 🔒 **[Private Registry Setup](docs/PRIVATE_REGISTRY_SETUP.md)** — развертывание через собственный Docker Registry (
  рекомендуется)
- ⚡ **[Быстрый деплой через Registry](QUICK_DEPLOY_REGISTRY.md)** — краткая инструкция за 5 шагов
- 🤖 **[GitHub Actions Setup](.github/GITHUB_ACTIONS_SETUP.md)** — настройка автоматического CI/CD
- [Полная инструкция по деплою](DEPLOYMENT.md) — развёртывание на VPS с автоматическим CI/CD
- [CI/CD Pipeline](.github/CI_CD.md) — автоматическое тестирование и деплой через GitHub Actions
...
3. Выберите **имя бота** (например: "Family Task Manager")
4. Выберите **username** (должен заканчиваться на `bot`, например: `family_task_manager_bot`)
...
### Настройка описания (опционально)
...
### 🧪 Тестирование PR перед слиянием (PR Preview)

Для тестирования изменений в изолированном окружении:

1. **Создайте PR** из вашей ветки
   ```bash
...
**Минимальная конфигурация** (Timeweb Cloud):
